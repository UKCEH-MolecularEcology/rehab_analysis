...
#        semibin=os.path.join(RESULTS_DIR, "bins/semibin/bins"),
...
#        semibinout=os.path.join(RESULTS_DIR, "bins/dastool/semibin_das.tsv")
    ...
    shell:
        """
        (date && mkdir -p $(dirname {output.metabatout}) && {params.src} -e fa -i {input.metabat} > {output.metabatout} && 
        perl -pe 's/metabat./{params.value}_metabat_/g' {output.metabatout} > t.txt && mv t.txt {output.metabatout} && 
        perl -pe 's/,/\t{params.value}_concoct_/g' {input.concoct} | tail -n +2 | sed 's/>s/s/g' > {output.concoctout} && date) 2> {log.err} > {log.out}
        """
...
#        semibin=rules.prepare_dasTool.output.semibinout,
    ...
    shell:
        "(date && "
        "{params.src} -i {input.concoct},{input.metabat} -l concoct,metabat -c {input.cont} -o $(dirname {output})/das --write_bins --search_engine diamond --threads {threads} && "
        "date) 2> {log.err} > {log.out}"        
      
# {params.src} -i {input.concoct},{input.metabat},{input.semibin} -l concoct,metabat,semibin -c {input.cont} -o $(dirname {output})/das --write_bins --search_engine diamond --threads {threads}
