...
rule file_prep:
    ...
    output:
        faa=os.path.join(RESULTS_DIR, "eukulele_input/{sid}.faa")
    log:
        os.path.join(RESULTS_DIR, "logs/eukulele/{sid}.log")
    message:
        "Symlinking {wildcards.sid} for eukulele"
    shell:
        "(date && cp -v {input.faa} {output.faa} && date) &> >(tee {log})"

rule eukulele:
    input:
        expand(os.path.join(RESULTS_DIR, "eukulele_input/{sid}.faa"), sid=SAMPLES.index)
    output:
    ...
    message:
        "Running EUKULELE"
    shell:
        "(date && EUKulele all -m mets --sample_dir $(dirname {input[0]}) --out_dir {output[0]} --database {params.db} --scratch {params.scratch} --CPUs {threads} --alignment_choice {params.aligner} --reference_dir {params.ref_dir} && "
        "date) &> >(tee {log})"
...
rule merge_cov:
    input:
        ...
        euks=os.path.join(RESULTS_DIR, "eukulele_output")
    output:
...
rule euk_abundances:
    input:
        expand(os.path.join(RESULTS_DIR, "euk_tax_cov/{sid}_eukaryotes.txt"), sid=SAMPLES.index)
    output:
        os.path.join(RESULTS_DIR, "euk_tax_cov/merged_eukaryote_abundances.txt")
    log:
...
rule euk_abundances_all:
    input:
        expand(os.path.join(RESULTS_DIR, "euk_tax_cov/{sid}_eukulele_ALL.txt"), sid=SAMPLES.index)
    output:
        os.path.join(RESULTS_DIR, "euk_tax_cov/merged_eukulele_all_abundances.txt")
    log:
