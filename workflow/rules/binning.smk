...
rule concoct_bins:
    input:
        cluster=os.path.join(RESULTS_DIR,"bins/concoct/concoct_clustering_merged.csv"),
        fa=os.path.join(RESULTS_DIR, "assembly/cat_assembly_filter.fasta")
    output:
        directory(os.path.join(RESULTS_DIR,"bins/concoct_bins"))
    conda:
        os.path.join(ENV_DIR, "concoct.yaml")
    log:
        out=os.path.join(RESULTS_DIR, "logs/concoct_bins.out.log"),
        err=os.path.join(RESULTS_DIR, "logs/concoct_bins.err.log")
    message:
        "Extracting concoct bins"
    shell:
        "(date && mkdir {output[0]} && "
        "extract_fasta_bins.py {input.fa} {input.cluster} --output_path {output} && "
        """cd {output[0]} && for filename in *.fa; do awk -v fname="concoct" '/^>/ {{print $0 "_" fname; next}} {{print}}' "$filename" | sed 's/>>s/>s/g/' > "temp.fa" && mv "temp.fa" "$filename"; done && """
        "date) 2> {log.err} > {log.out}"
...
#        bam=expand(os.path.join(RESULTS_DIR,"bam/{sid}/cat_assembly_{sid}.bam"), sid=SAMPLES.index)
#    output:
#        bins=directory(os.path.join(RESULTS_DIR, "bins/semibin/"))
#    conda:
#        os.path.join(ENV_DIR, "semibin.yaml")
#    log:
#        out=os.path.join(RESULTS_DIR, "logs/semibin.out.log"),
#        err=os.path.join(RESULTS_DIR, "logs/semibin.err.log")
#    params:
