...
rule seed:
    input:
        ...
        expand(os.path.join(RESULTS_DIR, "seed/{sid}/{sid}.reads_seed.reduced_counts.tsv"), sid=SAMPLES.index),
        expand(os.path.join(RESULTS_DIR, "preprocessed/reads/{sid}/{sid}_filtered.R1.fa"), sid=SAMPLES.index),
        os.path.join(RESULTS_DIR, "seed/abundance_tab.csv"),
        os.path.join(RESULTS_DIR, "seed/reads_abundance_tab.csv")
    output:
...
## READS ##
## Running SEED on forward reads alone similar to previous work by Briony
rule create_fasta:
    input:
        sr1=os.path.join(RESULTS_DIR, "preprocessed/reads/{sid}/{sid}_filtered.R1.fq")
    output:
        sr_fasta=os.path.join(RESULTS_DIR, "preprocessed/reads/{sid}/{sid}_filtered.R1.fa")
    log:
        os.path.join(RESULTS_DIR, "logs/seed/convert_fasta.{sid}.log")
    conda:
        os.path.join(ENV_DIR, "seqkit.yaml")
    message:
        "Converting concatenated SEED abundances to a matrix for: {wildcards.sid}"
    shell:
        "(date && fq2fa {input.sr1} -o {output.sr_fasta} && date) &> >(tee {log})"

use rule seed_diamond as reads_seed_diamond with:
    input:
        fasta=os.path.join(RESULTS_DIR, "preprocessed/reads/{sid}/{sid}_filtered.R1.fa")
    output:
        daa=os.path.join(RESULTS_DIR, "seed/{sid}/{sid}_reads_seed.daa"),
        tsv=os.path.join(RESULTS_DIR, "seed/{sid}/{sid}_reads_seed.tsv")
    log:
        os.path.join(RESULTS_DIR, "logs/seed/reads_seed_diamond.{sid}.log")
    message:
        "Running diamond on reads for: {wildcards.sid}"          

use rule gene_counts as reads_gene_counts with:
    input:
        rules.reads_seed_diamond.output.tsv
    output:
        counts=os.path.join(RESULTS_DIR, "seed/{sid}/{sid}.reads_seed.counts.tsv"),
        hits=os.path.join(RESULTS_DIR, "seed/{sid}/{sid}.reads_seed.per_hit.tsv")
    log:
        os.path.join(RESULTS_DIR, "logs/seed/reads_gene_counts.{sid}.log")
    message:
        "Getting reads-based SEED gene counts for: {wildcards.sid}"  

use rule reduce_counts as reads_reduce_counts with:
    input:
        rules.reads_gene_counts.output.counts
    output:
        reduced=os.path.join(RESULTS_DIR, "seed/{sid}/{sid}.reads_seed.reduced_counts.tsv")
    log:
        os.path.join(RESULTS_DIR, "logs/seed/reads_reduce_counts.{sid}.log")
    message:
        "Reducing reads-based concatenated SEED counts for: {wildcards.sid}"        

# Concatenating counts for all samples
rule reads_concatenate_abundance:
    input:
        counts=expand(os.path.join(RESULTS_DIR, "seed/{sid}/{sid}.reads_seed.reduced_counts.tsv"), sid=SAMPLES.index)
    output:
        os.path.join(RESULTS_DIR, "seed/allsamples_reads_anno_concatenated.tsv")
    log:
        os.path.join(RESULTS_DIR, "logs/seed/reads_concatenating_allsamples.log")
    message:
        "Concatenating read-based reduced SEED counts"
    run:
        # Get the input and output paths
        input_path = os.path.dirname(output[0])
        output_path = output[0]
        ...
        with open(output_path, "w") as out:
            for root, dirs, filenames in os.walk(input_path):
                for filename in filenames:
                    ...
                    # get files with ".reads_seed.reduced_counts.tsv" ending
                    if filename.endswith('.reads_seed.reduced_counts.tsv'):

                        # Get full file path of current file.
                        filename_path = os.path.join(root, filename)

                        # Get sample name from filename.
                        sample_name = filename.split(".reads_seed.reduced_counts.tsv")[0]
...
# Converting concatenated abundance to matrix
use rule matrify as reads_matrify with:
    input:
        rules.reads_concatenate_abundance.output[0]
    output:
        os.path.join(RESULTS_DIR, "seed/reads_abundance_tab.csv")
    log:
        os.path.join(RESULTS_DIR, "logs/seed/reads_matrify.log")
    message:
        "Converting reads-based concatenated SEED abundances to a matrix"
